...
object APNGSplitter {
    ...
    fun splitWithOpsApplied(apng: APNG, addDefaultImage: Boolean = false) : List<ByteArray> {
        ...
        for (i in frames.indices) {
            ...
            applyBlendOp(activeFrameImage, renderedImage, BlendOps.entries[activeFrame.fcTLChunk!!.blendOp.toInt()], region)

            val baos = ByteArrayOutputStream()
            ImageIO.write(renderedImage, "png", baos)
            out += baos.toByteArray()

            applyDisposeOp(lastFrameImage, renderedImage, DisposeOps.entries[activeFrame.fcTLChunk!!.disposeOp.toInt()], region)
